...
function getDataClientHead() {
	...
	// Sesuai dengan select#filter_kota_broadcast_head di HTML
		...
		success: function (data) {
			...
			if (data.data.length != 0) {
				...
				jQuery.each(data.data, function (i, val) {
					...
					// Tentukan status client berdasarkan bulan_selisih
					var status_client = '';
					if (val.bulan_selisih == 0 || val.bulan_selisih == null) {
						status_client = 'new';
					} else if (val.bulan_selisih > 2) {
					...
					// Tentukan warna row berdasarkan status broadcast log
					var warna_status = '';
					if (data.client_log[val.client_id] != null) {
						var log_status = data.client_log[val.client_id].status_broadcast;
						if (log_status == 'F' || log_status == 'A' || log_status == null) {
							warna_status = 'card-color-red';
						} else if (log_status == 'S') {
							warna_status = 'btn-color-greenWhite';
						} else if (log_status == 'D' || log_status == 'R') {
							warna_status = 'btn-color-blueWhite';
						}
					}

					// Bangun row dengan 4 kolom: No | Nama Client | Kota | Details
					data_client += '<tr class="' + warna_status + '">';
					data_client += '  <td align="center" class="label-cell" style="border-left:1px solid grey;border-bottom:1px solid grey;">' + no + '</td>';
					data_client += '  <td align="left" class="label-cell" style="border-left:1px solid grey;border-bottom:1px solid grey;">' + val.client_nama + '<input type="hidden" id="status_check_client_' + no + '" name="status_check_client_' + no + '" value="' + status_client + '" readonly></td>';
					data_client += '  <td align="left" class="label-cell" style="border-left:1px solid grey;border-bottom:1px solid grey;">' + kota + '</td>';
					data_client += '  <td style="border-left:1px solid grey;border-right:1px solid grey;border-bottom:1px solid grey;text-align:center;">';
				...
				// Tulis ke tabel & update counter — ID disesuaikan dengan HTML
