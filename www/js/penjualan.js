...
var isPrefillingStyle = false;
var fullColourPopupPerforma = null;
var currentHCCountPerforma = 1;
...
// ============================================================
// BANK HELPERS (dari Sales) — Task 6 Plan 1
// ============================================================

function generateBankOptionsByCode(selectedBankCode, excludeTunai) {
    selectedBankCode = selectedBankCode || 'Mandiri';
    excludeTunai = excludeTunai || false;
    var options = '';
    if (globalBankData.length === 0) {
    ...
    globalBankData.forEach(function(bank) {
        if (excludeTunai && bank.bank_kode === 'Tunai') return;
        var selected = (bank.bank_kode === selectedBankCode) ? ' selected' : '';
        options += '<option value="' + bank.bank_id + '"' + selected + '>' + bank.bank_label + '</option>';
    });
    return options;
}
...
function getBankIdByCode(bankCode) {
    if (!bankCode || bankCode === 'null' || bankCode === 'undefined') return null;
    var bank = globalBankData.find(function(b) { return b.bank_kode === bankCode; });
    return bank ? bank.bank_id : null;
}

function getBankLabel(bankIdOrCode) {
    if (!bankIdOrCode || bankIdOrCode === 'null' || bankIdOrCode === 'undefined' || bankIdOrCode === '') return '-';
    if (!isNaN(bankIdOrCode)) {
        var bankById = globalBankData.find(function(b) { return b.bank_id === parseInt(bankIdOrCode); });
        if (bankById) return bankById.bank_label;
        var hardcodedLabels = { 1:'BCA', 2:'BRI', 3:'Mandiri', 4:'Mandiri Bisnis', 5:'Tunai', 6:'Mandiri (Yu Shujin)' };
        if (hardcodedLabels[parseInt(bankIdOrCode)]) return hardcodedLabels[parseInt(bankIdOrCode)];
    }
    var bankByCode = globalBankData.find(function(b) { return b.bank_kode === bankIdOrCode; });
    if (bankByCode) return bankByCode.bank_label;
    return bankIdOrCode;
}

function getBankInfoById(bankId) {
    var defaultBank = { nama: 'Mandiri', rekening: '141 000 225 5818', atas_nama: 'Sutono' };
    ...
    var bank = globalBankData.find(function(b) { return b.bank_id === bankId; });
    if (bank) {
        return {
...
function generateBankInfoForInvoice(bankId) {
    var html = '';
    if (checkIsOwner() && bankId) {
        if (bankId === 'Mandiri Owner') bankId = 6;
        var bankInfo = getBankInfoById(parseInt(bankId));
        html += '      <tr>';
        html += '          <td style="border-bottom: solid 1px; border-top: solid 1px; border-left: solid 1px; padding:4px;" width="2%" align="left">' + bankInfo.nama + '</td>';
        html += '          <td style="border-bottom: solid 1px; border-top: solid 1px; padding:4px;" width="1%" align="left">:</td>';
        html += '          <td style="border-bottom: solid 1px; border-top: solid 1px; border-right: solid 1px; padding:2px;" width="47%" align="left">' + bankInfo.rekening + ' a.n ' + bankInfo.atas_nama + '</td>';
        html += '          <td width="13%" align="center"></td>';
        html += '      </tr>';
    } else {
        // html += '      <tr>';
        // html += '          <td style="border-top: solid 1px; border-left: solid 1px; padding:4px;" width="2%" align="left">BCA</td>';
        // html += '          <td style="border-top: solid 1px; padding:4px;" width="1%" align="left">:</td>';
        // html += '          <td style="border-top: solid 1px; border-right: solid 1px; padding:2px;" width="47%" align="left">01831 29551 a.n Sutono</td>';
        // html += '          <td width="13%" align="center"></td>';
        // html += '      </tr>';
        html += '      <tr>';
        html += '          <td style="border-bottom: solid 1px; border-top: solid 1px; border-left: solid 1px; padding:4px;" width="2%" align="left">Mandiri</td>';
        html += '          <td style="border-bottom: solid 1px; border-top: solid 1px; padding:4px;" width="1%" align="left">:</td>';
        html += '          <td style="border-bottom: solid 1px; border-top: solid 1px; border-right: solid 1px; padding:2px;" width="47%" align="left">141 000 225 5818 a.n Sutono</td>';
        html += '          <td width="13%" align="center"></td>';
        html += '      </tr>';
    }
    return html;
...
// ============================================================
// UTILITIES (dari Sales) — Task 3 & 4 Plan 1
// ============================================================
...
// Task 3 Plan 2: setSelectWhiteOnChoose
function setSelectWhiteOnChoose(selector) {
    var el = document.querySelector(selector);
    if (!el) return;
    if (el.value !== '' && el.value !== null) {
        el.classList.add('select-white-text');
    }
    el.addEventListener('change', function () {
        if (this.value !== '' && this.value !== null) {
            this.classList.add('select-white-text');
        } else {
            this.classList.remove('select-white-text');
        }
    });
}
...
function enableOngkirEdit($field, count, penjualan_id, originalValue) {
    console.log('Enable edit mode untuk ongkir_' + count);
    $field.css({ 'background-color': 'white', 'color': 'black', 'cursor': 'text', 'border': '2px solid #007aff' });
    $field.prop('readonly', false);
    $field.off('click');
    $field.focus();
    $field.select();
    $field.mask('000,000,000,000', { reverse: true });
    $field.on('blur', function() {
        var newValue = $field.val().replace(/,/g, '');
        if (newValue === originalValue.toString().replace(/,/g, '')) {
            cancelOngkirEdit($field, originalValue);
            return;
        }
        setTimeout(function() {
            app.dialog.confirm(
                '<div style="text-align:center; padding:10px;">' +
                '<h3 style="color:#ff3b30; margin-bottom:15px;">⚠️ Konfirmasi Penyimpanan</h3>' +
                '<p style="font-size:16px; margin-bottom:10px;">Apakah Anda yakin ingin menyimpan perubahan ini?</p>' +
                '<p style="font-size:14px; color:#8e8e93; margin-bottom:15px;"><b>Data tidak dapat diubah lagi setelah disimpan!</b></p>' +
                '<div style="background-color:#2c2c2e; padding:10px; border-radius:8px; margin:10px 0;">' +
                '<p style="font-size:14px; margin:5px 0;"><b>Nilai Asli:</b> <span style="color:#ff3b30;">' + number_format(originalValue) + '</span></p>' +
                '<p style="font-size:14px; margin:5px 0;"><b>Nilai Baru:</b> <span style="color:#34c759;">' + number_format(newValue) + '</span></p>' +
                '</div></div>',
                function() { saveOngkirEdit($field, count, penjualan_id, originalValue, newValue); },
                function() { app.dialog.alert('Perubahan dibatalkan', function() { cancelOngkirEdit($field, originalValue); }); }
            );
        }, 200);
    });
}

function cancelOngkirEdit($field, originalValue) {
    $field.val(number_format(originalValue));
    $field.css({ 'background-color': '#e5e5e7', 'color': '#3a3a3c', 'cursor': 'pointer', 'border': 'none' });
    $field.prop('readonly', true);
    $field.off('blur');
    var count = $field.attr('id').replace('ongkir_', '');
    var penjualan_id = $field.attr('data-penjualan-id');
    var statusOngkir = $field.attr('data-status-ongkir') || 'pending';
    $field.on('click', function() {
        handleOngkirClick(this, count, penjualan_id, originalValue, statusOngkir);
    });
}
	...
	jQuery.ajax({
		...
		success: function (data) {
			...
			if (data.data.length != 0) {
				...
				var biaya_kirim_val = parseFloat(data.data[0].biaya_kirim || data.data[0].ongkir || 0);
				if (biaya_kirim_val != 0) {
					invoice_penjualan += '		<tr>';
					invoice_penjualan += '			<td colspan="4" style="font-weight:bold;" align="right"></td>';
					invoice_penjualan += '			<td colspan="1" style="border-top: solid 1px; border-left: solid 1px; font-weight:bold;" align="left">';
					...
					invoice_penjualan += '			</td>';
					invoice_penjualan += '			<td colspan="1" style="border-top: solid 1px; border-left: solid 1px; border-right: solid 1px;  font-weight:bold;" align="left">';
					invoice_penjualan += '			<font style="float:right;">' + number_format(biaya_kirim_val) + '</font>';
					invoice_penjualan += '			</td>';
					invoice_penjualan += '		</tr>';
				}
				...
				if (number_format(data.data[0].pembayaran_1) != 0) {
					invoice_penjualan += '   <font style="float:right;">' + number_format(parseFloat(penjualan_total) + biaya_packing_val + biaya_kirim_val - parseFloat(data.data[0].pembayaran_1)) + '</font>';
				} else {
					invoice_penjualan += '   <font style="float:right;">' + number_format(parseFloat(penjualan_total) + biaya_packing_val + biaya_kirim_val) + '</font>';
				}
				invoice_penjualan += '			</td>';
				invoice_penjualan += '		</tr>'
...
// ============================================================
// SPK 3 LEMBAR (dari Sales) — Task 1 Plan 3
// ============================================================
		...
		success: function (data) {
			if (data.data.length != 0) {
				jQuery.each(data.data, function (i, val) {
					...
					spk1_data += ' <tr>';
					spk1_data += '  <td colspan="3" width="50%" class="label-cell" style="border-left: solid 1px; "><center>' + val.penjualan_jenis + '<br><img src="' + path_image + '/' + val.gambar + '" width="70%"></center></td>';
				...
				success: function (data) {
					if (data.data.length != 0) {
						jQuery.each(data.data, function (i, val) {
							...
							spk2_data += ' <tr>';
							spk2_data += '  <td colspan="3" width="50%" class="label-cell" style="border-left: solid 1px; "><center>' + val.penjualan_jenis + '<br><img src="' + path_image + '/' + val.gambar + '" width="70%"></center></td>';
						...
						success: function (data) {
							if (data.data.length != 0) {
								jQuery.each(data.data, function (i, val) {
									...
									spk3_data += ' <tr>';
									spk3_data += '  <td colspan="3" width="50%" class="label-cell" style="border-left: solid 1px; "><center>' + val.penjualan_jenis + '<br><img src="' + path_image + '/' + val.gambar + '" width="70%"></center></td>';
	...
	jQuery.ajax({
		...
		beforeSend: function () {
			...
			if (penjualan_status_pembayaran == "Lunas") {
				var style_table = "background-image:url('img/logo/lunas.jpg'); border:none;  background-position: center;  background-size:45% auto; margin:0px; background-repeat:no-repeat";
			} else {
...
// Task 2 Plan 2: firstShowPerformaEdit — versi Sales (deployStyleToSelectFromDB)
// NOTE: proforma_old_material_edit_1, proforma_old_hex_edit_1, selected_hex_color_edit
//       belum ada di HTML CS — bagian hex/chip tidak aktif sampai elemen HTML ditambahkan.
function firstShowPerformaEdit() {
    isPrefillingStyle = true;
    setTimeout(function () {
        var style    = jQuery('#proforma_old_style_edit_1').val();
        var ukuran   = jQuery('#proforma_old_ukuran_edit_1').val();
        var material = jQuery('#proforma_old_material_edit_1').val();
        var hex      = jQuery('#proforma_old_hex_edit_1').val();

        if (material) jQuery('#material_edit').val(material).trigger('change');
        jQuery('#ukuran_edit_hc_1').val(ukuran).trigger('change');

        if (style && style.trim() !== '') {
            deployStyleToSelectFromDB(style, hex, 1);
        } else {
            jQuery('#style_hc_edit_input_1').text('Pilih Style');
            jQuery('#style_hc_edit_1').val([]).trigger('change');
            if (jQuery('#selected_hex_color_edit').length) jQuery('#selected_hex_color_edit').val('');
            if (jQuery('#show_color_fullcolor_edit').length) jQuery('#show_color_fullcolor_edit').empty();
        }

        isPrefillingStyle = false;
    }, 1000);
}

// === Task 1 Plan 2: Style HC Helper Functions (dari Sales) ===
...
function parseStyleString(raw) {
    ...
    if (parts.length <= 1) {
        parts = base.split(/\s+dan\s+/i).map(function(s) { return s.trim(); }).filter(Boolean);
    }
    return parts;
}

function normalizePlusPrefix(s) {
    return String(s).replace(/^\+\s+/, '+');
}

function keyNormalize(label) {
    var v = String(label).trim();
    v = normalizePlusPrefix(v);
    if (/^Full\s*colou?r\b/i.test(v)) {
        v = v.replace(/\s*\([^)]*\)\s*$/, '');
        v = v.replace(/^Full\s*colou?r\b/i, 'Asesoris F.C');
    }
    v = v.replace(/^[\s]*(•|-)\s*/, '');
    v = v.replace(/\s+/g, ' ').trim();
    if (/^Asesoris\s*F\.?C\b/i.test(v)) {
        v = 'Asesoris F.C';
    }
    return v.toLowerCase();
}

function extractParenColor(styleRaw) {
    var s = String(styleRaw);
    var m = s.match(/(?:Full\s*colou?r|Asesoris\s*F\.?C)\s*\(([^)]+)\)/i);
    return m ? m[1].trim() : '';
}

function deployStyleToSelectFromDB(styleRaw, hex, idx) {
    var $sel = jQuery('#style_hc_edit_' + idx);

    var optionKeyToActualValue = {};
    $sel.find('option').each(function () {
        var textKey  = keyNormalize(jQuery(this).text());
        var valueKey = keyNormalize(jQuery(this).val());
        var actual   = jQuery(this).val();
        if (textKey)  optionKeyToActualValue[textKey]  = actual;
        if (valueKey) optionKeyToActualValue[valueKey] = actual;
    });

    var labelsFromDB = parseStyleString(styleRaw);
    var desiredKeys  = labelsFromDB.map(keyNormalize);
    var selectedActualValues = [];
    desiredKeys.forEach(function(k) {
        if (optionKeyToActualValue[k] !== undefined) {
            selectedActualValues.push(optionKeyToActualValue[k]);
        }
    });

    $sel.val(selectedActualValues).trigger('change');

    var summaryParts = [];
    $sel.find('option:selected').each(function () {
        var txt = jQuery(this).text().trim();
        if (/^(Full\s*colou?r|Asesoris\s*F\.?C)\b/i.test(txt)) {
            summaryParts.push('Asesoris F.C');
        } else {
            summaryParts.push(txt);
        }
    });
    jQuery('#style_hc_edit_input_' + idx).text(summaryParts.join(', '));

    var $li = jQuery('#el_style_hc_edit_' + idx);
    if ($li.length) {
        var $outer = $li.children('.item-inner').first();
        $outer.next('.style-color-chip-holder').remove();
        if (hex) {
            if (jQuery('#selected_hex_color_edit').length) jQuery('#selected_hex_color_edit').val(hex);
            var colorName = extractParenColor(styleRaw);
            if (jQuery('#proforma_old_style_color_edit_1').length) jQuery('#proforma_old_style_color_edit_1').val(colorName);
            jQuery(
                '<div class="style-color-chip-holder" style="display:flex;align-items:center;margin-top:6px;margin-right:6px;">' +
                '<span class="swatch-chip-performa" style="display:inline-block;vertical-align:middle;width:20px;height:20px;border-radius:4px;background:' + hex + ';"></span>' +
                '</div>'
            ).insertAfter($outer);
        } else {
            if (jQuery('#selected_hex_color_edit').length) jQuery('#selected_hex_color_edit').val('');
    ...
    var ss = app.smartSelect.get('#ss_style_hc_edit_' + idx)
          || app.smartSelect.get('#ss_style_edit')
          || app.smartSelect.get('#ss_style_hc_' + idx);
    if (ss) ss.setValue($sel.val() || []);
}

function escapeHtmlPerforma(s) {
    return String(s).replace(/[&<>"']/g, function (m) {
        return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[m]);
    });
}

function changeWarnaFullColorPerforma(count) {
    if (isPrefillingStyle) return;
    var $sel   = jQuery('#style_hc_' + count + '_1');
    var values = ($sel.val() || []);
    var hasFull = Array.isArray(values)
        ? values.some(function(v) { return /^Full\s*colour/i.test(String(v)); })
        : /^Full\s*colour/i.test(String(values));
    var ss = app.smartSelect.get('#ss_style_' + count);
    if (hasFull) {
        setTimeout(function() {
            if (ss) ss.close();
            selectBoxStyleFullColorPerforma(count);
        }, 900);
    } else {
        if (ss) ss.close();
        jQuery('#selected_color_' + count).val('');
        jQuery('#selected_hex_color_' + count).val('');
        removeChipHolder(count);
    }
}

function selectBoxStyleFullColorPerforma(idx) {
    // NOTE: Membutuhkan popup .fullcolour-performa-penjualan-popup di HTML
    // Belum ada di CS penjualan.html — akan aktif setelah HTML popup ditambahkan
    if (!document.querySelector('.fullcolour-performa-penjualan-popup')) return;
    currentHCCountPerforma = idx;
    var produk_id = jQuery('#jenis_' + idx + '_popup').val();
    if (!fullColourPopupPerforma) {
        fullColourPopupPerforma = app.popup.create({
            el: '.fullcolour-performa-penjualan-popup',
            closeByBackdropClick: false,
            closeOnEscape: false,
            swipeToClose: false,
        });
    }
    var $grid = jQuery('#swatch-container-performa');
    ...
    jQuery.ajax({
        ...
        success: function (res) {
            ...
            if (!list.length) {
                ...
                fullColourPopupPerforma.open();
                return;
            }
            var html = list.map(function (v) {
                var hex  = v.produk_grup_fullcolor || '';
                var name = v.nama_fullcolor || '';
                return '<button type="button" class="swatch-item-performa" data-hex-performa="' + escapeHtmlPerforma(hex) + '" data-name-performa="' + escapeHtmlPerforma(name) + '">' +
                    '<span class="swatch-chip-performa" style="background:' + hex + ';width:20px;height:20px;border-radius:4px;display:inline-block;margin-right:8px;"></span>' +
                    '<span class="swatch-name-performa">' + escapeHtmlPerforma(name) + '</span></button>';
            }).join('');
            $grid.append(html);
            fullColourPopupPerforma.open();
        },
...
jQuery(document)
    .off('click', '.swatch-item-performa')
    .on('click', '.swatch-item-performa', function () {
        var hex  = String(jQuery(this).data('hex-performa') || '').trim();
        var name = String(jQuery(this).data('name-performa') || '').trim();
        var idx  = currentHCCountPerforma || 1;
        jQuery('#selected_color_' + idx).val(name).trigger('change');
        jQuery('#selected_hex_color_' + idx).val(hex);
        removeChipHolder(idx);
        var chipHtml = '<div class="style-color-chip-holder" style="display:flex;align-items:center;margin-top:6px;margin-right:6px;">' +
            '<span class="swatch-chip-performa" style="display:inline-block;vertical-align:middle;background:' + hex + ';width:20px;height:20px;border-radius:4px;"></span>' +
            '</div>';
        var $li = jQuery('#el_style_hc_' + idx + '_1');
        if ($li.length) {
            var $outer = $li.children('.item-inner').first();
            if ($outer.length) {
                var $nextHolder = $outer.next('.style-color-chip-holder');
                if ($nextHolder.length) {
                    $nextHolder.find('.swatch-chip-performa').css('background', hex);
                } else {
                    jQuery(chipHtml).insertAfter($outer);
                }
            }
        }
        jQuery('#close-fullcolour-performa-penjualan-popup').click();
    });

function removeChipHolder(idx) {
    var $li = jQuery('#el_style_hc_' + idx + '_1');
    if ($li.length) {
        var $outer = $li.children('.item-inner').first();
        if ($outer.length) {
...
function invoicePenjualan(performa_header_id, biaya_kirim, client_alamat, client_cp, client_cp_posisi, client_id, client_kota, client_nama, client_telp, jenis_penjualan, karyawan_id, penjualan_global_diskon, penjualan_grandtotal, penjualan_id, penjualan_jumlah_pembayaran, penjualan_keterangan, penjualan_status, penjualan_status_pembayaran, penjualan_tanggal, penjualan_tanggal_kirim, penjualan_total, penjualan_void_keterangan, penjualan_total_qty, sisa_value, extra, packing) {
	console.log('invoicePenjualan called with:');
	// Format label packing untuk dokumen
	var packingLabels = { 'polos': 'Polos', 'plastik': 'Plastik', 'kardus': 'Kardus' };
	var packing_label = packing && packing !== '0' ? 'Packing ' + (packingLabels[packing] || packing.charAt(0).toUpperCase() + packing.slice(1)) : 'Packing finishing plastic';
							...
							if (data.data.length != 0) {
								...
								jQuery.ajax({
									...
									success: function (data) {
										...
										invoice_penjualan += ' 					<img src="img/logo/invoiceLogo.png" style="opacity: 0.6;" width="100" height="100">';
		...
		success: function (data) {
			...
			if (data.data.length != 0) {
				$.each(data.data, function (i, item) {
					...
					penjualan_value += '<td align="center" width="25%" style="border-left:1px solid gray;  border-right:1px solid gray; border-bottom:1px solid gray;"  class="label-cell">' + (item.foto_produksi_sjc != null ? 'Xinyao' : '-') + '</td>';
					penjualan_value += '<td align="center" width="25%" style="border-left:1px solid gray;  border-right:1px solid gray; border-bottom:1px solid gray;"  class="label-cell"><button data-popup=".view-foto-sjc" onclick="viewFotoSjc(\'' + item.foto_produksi_sjc + '\', \'' + penjualan_id + '\', \'' + item.penjualan_tanggal + '\')" class="popup-open ' + color_btn_sjc_id + ' button-small col button text-bold">Foto</button></td>';
					penjualan_value += '</tr>'
				});
			} else {
				app.dialog.alert('Tidak Ada Surat Jalan');
	...
	jQuery.ajax({
		...
		success: function (data) {
			...
			if (data.data.length != 0) {
				...
				proforma_data += ' 					<img src="img/logo/invoiceLogo.png" style="opacity: 0.6;" width="100" height="100">';
				...
				if (data.data[0].bank_id && data.data[0].bank_label) {
					...
					console.log('Tidak ada bank dipilih, tampilkan default Mandiri');

					// Default: tampilkan Mandiri saja
					bankInfo += '      <tr>';
					bankInfo += '          <td style="border-bottom: solid 1px; border-top: solid 1px; border-left: solid 1px; padding:4px;" width="2%" align="left" class="">Mandiri</td>';
					bankInfo += '          <td style="border-bottom: solid 1px;  border-top: solid 1px; padding:4px;" width="1%" align="left" class="">:</td>';
					bankInfo += '          <td style="border-bottom: solid 1px;  border-top: solid 1px;  border-right: solid  1px; padding:2px;" width="47%" align="left" class="">141 000 225 5818 a.n Sutono</td>';
