...
function detailPenjualan(dt_record, penjualan_id, client_cp, client_cp_posisi, client_id, client_kota, client_nama, client_telp, jenis_penjualan, karyawan_id, karyawan_nama, penjualan_global_diskon, penjualan_grandtotal, penjualan_id, penjualan_jumlah_pembayaran, penjualan_keterangan, penjualan_status, penjualan_status_pembayaran, penjualan_tanggal, penjualan_tanggal_kirim, penjualan_total, penjualan_void_keterangan, penjualan_total_qty, tgl_cs_deadline, lokasi_pabrik, wilayah) {
	...
	if (tgl_cs_deadline != 'null' && tgl_cs_deadline != null) {
		...
		detail_cs_button += '	<i onclick="updateUrgent(\'' + penjualan_id + '\');"';
			...
			$.each(data.data.data, function (i, item) {
				...
				if (arsip_value == 'empty') {
					...
					if (sisa_kirim_sj > 0 || sisa > 0) {
						...
						penjualan_value += '   <button  class="' + btn_alamat_kirim_penjualan + ' button-small col button popup-open text-bold" data-popup=".input-alamat-kirim" onclick="kirimAlamat(\'' + item.penjualan_id + '\');">Shipment</button>';
						penjualan_value += '</td>';
						...
						penjualan_value += '<td style="border-right:1px solid gray; border-bottom:1px solid gray;" class="label-cell">';
						penjualan_value += '   <button  class="text-add-colour-black-soft bg-dark-gray-young button-small col button text-bold popup-open" data-popup=".detail-report-popup" onclick="fullReport(\'' + item.penjualan_id_primary + '\',\'' + item.performa_id_relation + '\',\'' + item.performa_id_relation + '\',\'' + item.biaya_kirim + '\',\'' + item.client_alamat + '\',\'' + item.client_cp + '\',\'' + item.client_cp_posisi + '\',\'' + item.client_id + '\',\'' + item.client_kota + '\',\'' + item.client_nama + '\',\'' + item.client_telp + '\',\'' + item.jenis_penjualan + '\',\'' + item.karyawan_id + '\',\'' + item.penjualan_global_diskon + '\',\'' + item.penjualan_grandtotal + '\',\'' + item.penjualan_id + '\',\'' + item.penjualan_jumlah_pembayaran + '\',\'' + item.penjualan_keterangan + '\',\'' + item.penjualan_status + '\',\'' + item.penjualan_status_pembayaran + '\',\'' + item.penjualan_tanggal + '\',\'' + item.penjualan_tanggal_kirim + '\',\'' + item.penjualan_total + '\',\'' + item.penjualan_void_keterangan + '\',\'' + item.penjualan_total_qty + '\');">Report</button>';
						penjualan_value += '</td>';
						// if (item.valid_cs == 2) {
						if (item.is_edit == 1) {
							penjualan_value += '<td style="border-right:1px solid gray; border-bottom:1px solid gray;" class="label-cell">';
...
function updateUrgent(penjualan_id) {
	app.dialog.create({
		title: 'Update Status Urgent',
		text: 'Apakah Anda Yakin Mengubah Status Urgent ? ',
		cssClass: 'custom-dialog',
		closeByBackdropClick: 'true',
		buttons: [
			{
						...
						success: function (data) {
							...
							if (data.status == 'success') {
								app.dialog.alert('Berhasil Update Urgent', function () {
									$("#close-detail-sales").click();
									getPenjualanHeader(1);
								});
							} else {
								app.dialog.alert('Gagal Update Urgent', function () {
									$("#close-detail-sales").click();
									getPenjualanHeader(1);
								});
							}
		...
		success: function (data) {
			...
			if (data.data.length != 0) {
				...
				$.each(data.data, function (i2, item2) {
					...
					performa_value += '<td class="label-cell" style="background-color:' + color_tr + '; border-right:1px solid gray; border-bottom:1px solid gray;">';
					performa_value += '<img class="popup-open" data-popup=".detail-proforma-popup" onclick="penjualanGetPerformaDownload(\'' + item2.performa_header_id + '\',\'' + item2.karyawan_id + '\',\'' + item2.client_kota + '\',\'' + item2.client_nama + '\',\'' + item2.dt_record + '\',\'' + item2.performa_total_qty + '\',\'' + item2.extra + '\');" src="img/logo/donwloadproforma.png" width="80px" />';
					performa_value += '</td>';
